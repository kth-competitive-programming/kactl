...
template <int M> vector<ll> convMod(const vi &a, const vi &b) {
	...
	rep(i, 0, n) {
		int j = (n - i) & (n - 1);
		outl[-i & (n - i)] = (inl[i] + conj(inl[j])) * 0.5 * inr[i];
		outs[-i & (n - i)] = (inl[i] - conj(inl[j])) * -.5i * inr[i];
	}
	fft(outl, n, L), fft(outs, n, L);
	rep(i, 0, sz(res)) {
		outl[i] /= n, outs[i] /= n;
		ll av = ll(round(outl[i].real())) % M;
		ll bv = ll(round(outl[i].imag()) + round(outs[i].real())) % MOD;
		ll cv = ll(round(outs[i].imag())) % M;
		res[i] = ((av * cut + bv) * cut + cv) % MOD;
	}
	return res;
}